# ThisIsMISIS - Grave Rave

## 📑 Оглавление

- [🎵 GRAVE RAVE — Описание проекта](#-grave-rave)
- [🎮 Как поиграть](#-как-поиграть)
- [🧩 Основной функционал проекта](#-основной-функционал-проекта)
- [🛠 Технологии и инструменты](#-технологии-и-инструменты)
- [👥 Команда проекта](#-команда-проекта)
- [🎞 Демонстрация игры](#-демонстрация-игры)
  - [🎼 Основная механика](#-основная-механика)
  - [🧠 Прогрессия и тактика](#-прогрессия-и-тактика)
  - [✨ Фишки и уникальность](#-фишки-и-уникальность)
  - [▶ Онлайн-демо](#онлайн-демо)
- [🖥 Презентация проекта](#-презентация-проекта)
- [✔ Заключение](#-заключение)


# 🎵 GRAVE RAVE  
Музыкальный vampire-like с уникальной ритм-механикой, в которой каждая нота — это атака, а каждая мелодия — оружие.  
Игра была создана в рамках хакатона **Синеус 2025** по теме **«Ритм и Хаос»**.

---

## 🎮 Как поиграть

### 🔹 Вариант 1 — скачать .exe и запустить  
Скачать сборку можно по ссылке:

👉 **[GraveRave.exe (Google Drive)](https://drive.google.com/file/d/1gAOWNwjqXQl7EcawLYfj_GSCJHQbhZEh/view?usp=sharing)**  

После скачивания просто запустите файл — установка не требуется.

### 🔹 Вариант 2 — играть онлайн  
👉 https://geskawary234.itch.io/graverave

---

## 🧩 Основной функционал проекта

- Уникальная механика ритма: сетка 2×8 как музыкальный секвенсор.  
- Игра движется по ритму, автоматически проигрывая ноты-атаки.  
- Прогрессия уровней: новые ноты, звуки, модификаторы и типы врагов.  
- Выбор стартовой колоды.  
- Обучение встроено в процесс игры.  
- Начальная заставка и финальная катсцена + полноценная концовка.  
- Возможность рестарта и стабильный игровой цикл.  
- Цельная визуальная, звуковая и атмосферная подача.

---

## 🛠 Технологии и инструменты

**Игровой движок:**  
![Godot](https://img.shields.io/badge/Godot%20Engine-478CBF?logo=godot-engine&logoColor=white)

**Создание катсцен:**  
![After Effects](https://img.shields.io/badge/After%20Effects-9999FF?logo=adobeaftereffects&logoColor=white)

**Звук и музыка:**  
![FL Studio](https://img.shields.io/badge/FL%20Studio-F79F1F?logo=flstudio&logoColor=white)

**Отрисовка персонажей и арт:**  
![GoPaint](https://img.shields.io/badge/GoPaint-000000?logoColor=white)

---

## 👥 Команда проекта

| Участник | Роль | Контакты |
|---------|------|----------|
| **Писков Макар** | Тимлид, ПМ | [![tg](https://img.shields.io/badge/Telegram-2CA5E0?logo=telegram&logoColor=white)](https://t.me/makqar) |
| **Кеппель Юрий** | Разработчик | [![tg](https://img.shields.io/badge/Telegram-2CA5E0?logo=telegram&logoColor=white)](https://t.me/mircal) |
| **Драгович Данило** | Разработчик | [![tg](https://img.shields.io/badge/Telegram-2CA5E0?logo=telegram&logoColor=white)](https://t.me/Geskawary234) |
| **Сафронов Илья** | Звукорежиссёр, монтажёр | [![tg](https://img.shields.io/badge/Telegram-2CA5E0?logo=telegram&logoColor=white)](https://t.me/fffunnyvalentine) |
| **Федрушкова Юлия** | 2D-художник | [![tg](https://img.shields.io/badge/Telegram-2CA5E0?logo=telegram&logoColor=white)](https://t.me/tglrep) |

---

# 🎞 Демонстрация игры

## 🎼 Основная механика

Основная механика: **сетка 2×8, как музыкальный секвенсор**.  
Игра двигается по ритму — **слева направо, колонка за колонкой**.  
Когда колонка активируется — **все ноты в ней проигрываются и запускают атаки**.

![core-mechanic](./media/gif_attack.gif)  
*Гифка демонстрации основной механики*


---

## 🧠 Прогрессия и тактика

- С каждым уровнем игрок выбирает **новую ноту**, расширяя возможности.  
- Постепенно открываются **новые звуки** и **новые типы врагов**.  
- Чтобы выжить — нужно **перестраивать ритм под новых противников**, адаптируя стратегию.

![progression](./media/gif_progression.gif)  
*Гифка демонстрации прогрессии и тактики*


---

## ✨ Фишки и уникальность

Ниже — ключевые особенности игры.  
Каждый пункт сопровождается отдельной GIF-демонстрацией.  
Блоки отображаются в одной строке благодаря HTML.

<div style="display: flex; flex-wrap: wrap; gap: 20px;">

  <div style="flex: 1; min-width: 250px;">
    <h4>🎯 Уникальная механика атаки</h4>
    <img src="./media/gif_attack.gif" width="100%" />
  </div>

  <div style="flex: 1; min-width: 250px;">
    <h4>🎬 Начальная заставка и финальная катсцена</h4>
    <img src="./media/gif_cutscenes.gif" width="100%" />
  </div>

  <div style="flex: 1; min-width: 250px;">
    <h4>👾 Разнообразие атак и врагов</h4>
    <img src="./media/gif_attack.gif" width="100%" />
  </div>

</div>

<br>

<div style="display: flex; flex-wrap: wrap; gap: 20px;">

  <div style="flex: 1; min-width: 250px;">
    <h4>🔁 Возможность рестарта</h4>
    <img src="./media/gif_restart.gif" width="100%" />
  </div>

  <div style="flex: 1; min-width: 250px;">
    <h4>🏁 Есть концовка игры</h4>
    <img src="./media/gif_ending.gif" width="100%" />
  </div>

  <div style="flex: 1; min-width: 250px;">
    <h4>🧩 Модификаторы атак</h4>
    <img src="./media/gif_modifiers.gif" width="100%" />
  </div>

</div>

<br>

<div style="display: flex; flex-wrap: wrap; gap: 20px;">

  <div style="flex: 1; min-width: 250px;">
    <h4>🎵 Выбор начальной колоды</h4>
    <img src="./media/gif_deck.gif" width="100%" />
  </div>

  <div style="flex: 1; min-width: 250px;">
    <h4>📚 Обучение по ходу игры</h4>
    <img src="./media/gif_tutorial.gif" width="100%" />
  </div>

  <div style="flex: 1; min-width: 250px;">
    <h4>💫 Визуальные приятности</h4>
    <img src="./media/gif_fx.gif" width="100%" />
  </div>

</div>

---

### Онлайн-демо  
https://geskawary234.itch.io/graverave

---

## 🖥 Презентация проекта

Слайды, использованные для защиты на хакатоне:

👉 **[Презентация Grave Rave (PPTX) ](https://docs.google.com/presentation/d/1Y3geRWHNlirQPVpk-3rf0PyH5JR2I60n/edit?usp=sharing&ouid=105959865097520145024&rtpof=true&sd=true)**


## ✔ Заключение

**GRAVE RAVE** — это завершённая, атмосферная игра с сильной идеей: игрок сам создаёт ритм, а игра превращает его в оружие. Мы объединили музыку, стратегию и хаос, чтобы создать необычный и запоминающийся игровой опыт.

**Потенциальные улучшения:**
- новые зоны, боссы и типы врагов;  
- расширенные музыкальные комбинации и ритм-паттерны;  
- улучшенные катсцены и визуальные эффекты;  
- большая кампания и мета-прогрессия.

---

