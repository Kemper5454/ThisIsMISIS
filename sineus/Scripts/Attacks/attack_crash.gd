...
func play(beat: int):
	for i in 3:
		var slot = G.main.player.grid.get_slot(get_slot().get_index_of_slot()[1],get_slot().get_index_of_slot()[0]+i+1)
		if slot:
			var attack = slot.get_attack()
			if attack:
				attack.bonus_damage += 4-i
	
	var pitch = 1
	if is_tonal: 
		pitch = 2 ** ((id_to_note(beat))/12.0)
	Sounds.play_sound("crash", -10.0, "Music", 0.0, pitch) 
